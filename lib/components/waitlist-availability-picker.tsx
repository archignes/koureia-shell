...
export function WaitlistAvailabilityPicker({
  ...
  useEffect(() => {
    ...
    function updateScrollHint() {
      if (!scroller) return
      setCanScrollMore(scroller.scrollLeft + scroller.clientWidth < scroller.scrollWidth - 4)
    }
  ...
  return (
    ...
    >
      ...
      <div className="relative">
        <div
          ref={scrollRef}
          className="overflow-x-auto pb-1 [scrollbar-width:thin] [scrollbar-color:rgba(199,164,106,0.45)_rgba(228,231,239,0.08)]"
        >
          <div
            className="grid min-w-[22rem] gap-0.5 sm:min-w-[29rem] sm:gap-1"
            style={{ gridTemplateColumns: `3.9rem repeat(${days.length}, minmax(2.55rem, 1fr))` }}
          >
          ...
          {days.map((day) => {
            ...
            return (
              <button
                ...
                type="button"
                className={cn(
                  "min-h-9 rounded-md border px-1 py-1.5 text-center text-[0.64rem] leading-tight transition-colors sm:min-h-11 sm:rounded-lg sm:px-2 sm:py-2 sm:text-[0.72rem]",
                  active
          ...
          {rows.flatMap((minutes, rowIndex) => [
            <div
              key={`label-${minutes}`}
              className="sticky left-0 z-10 flex min-h-9 items-center justify-end whitespace-nowrap bg-[rgba(15,17,23,0.96)] pr-2 text-[0.58rem] text-[var(--shell-text-subtle)] sm:min-h-11 sm:text-[0.68rem]"
            >
            ...
            ...days.map((day, dayIndex) => {
              ...
              return (
                <button
                  ...
                  className={cn(
                    "min-h-9 rounded-md border transition-colors sm:min-h-11",
                    selectable
        ...
        {canScrollMore ? (
          <>
            <div
              aria-hidden="true"
              className="pointer-events-none absolute inset-y-0 right-0 w-10 bg-[linear-gradient(90deg,rgba(15,17,23,0),rgba(15,17,23,0.94))]"
            />
            <div className="pointer-events-none absolute right-2 top-1/2 -translate-y-1/2 rounded-full border border-[var(--shell-border-strong)] bg-[rgba(15,17,23,0.92)] px-2 py-1 text-[0.65rem] font-semibold text-[var(--shell-text-muted)] shadow-[0_8px_20px_rgba(0,0,0,0.28)]">
              Scroll →
            </div>
          </>
        ) : null}
